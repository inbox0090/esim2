            ...
            {filtered.map(t => (
              <div key={t.id} onClick={() => setSelected(t.id)}
                className={`p-4 border-b border-gray-50 cursor-pointer transition
                  ${selected === t.id ? 'bg-sky-50 border-l-2 border-l-sky-500' : 'hover:bg-gray-50'}`}>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                {selectedTicket.replies.map((r, i) => (
                  <div key={i} className="bg-sky-50 rounded-xl border border-sky-100 p-4 ml-8">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-7 h-7 gradient-bg rounded-full flex items-center justify-center text-white text-xs font-bold">A</div>
                      <span className="text-xs font-semibold text-sky-700">Admin</span>
              ...
              {selectedTicket.status === 'open' && (
                <div className="bg-white border-t border-gray-100 p-4">
                  <div className="flex gap-3">
                    <textarea className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-sky-400 bg-gray-50"
