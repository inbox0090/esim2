              ...
              {esims.map(e => {
                ...
                return (
                  <tr key={e.id} className="border-t border-gray-50 hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono text-[10px] text-purple-600">{e.iccid}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden" style={{minWidth:60}}>
                          <div className={`h-full rounded-full ${pct > 80 ? 'bg-red-400' : 'bg-purple-500'}`} style={{width:`${pct}%`}}></div>
                        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div className="grid grid-cols-2 gap-4 mb-6">
          {familyMembers.map(m => {
            ...
            const over = pct > 80;
            const color = over ? 'bg-red-400' : pct > 60 ? 'bg-amber-400' : 'bg-purple-500';
            return (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="flex gap-3">
            <input className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-400" placeholder="Vardas" value={name} onChange={e=>setName(e.target.value)} />
            <input type="number" className="w-28 border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-400" placeholder="GB planas" value={gb} onChange={e=>setGb(e.target.value)} />
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-100">
              {[
                { label: 'MB šiandien', val: '247', color: 'text-purple-600' },
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden relative" style={{minHeight:400}}>
            <div className="absolute inset-0" style={{background:'#e9ecf3', backgroundImage:'linear-gradient(rgba(99,102,241,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(99,102,241,0.08) 1px,transparent 1px)', backgroundSize:'30px 30px'}}>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <div className="w-5 h-5 bg-purple-600 rounded-full shadow-lg" style={{boxShadow:'0 0 0 0 rgba(124,58,237,0.4)', animation:'ping 1.5s ease-in-out infinite'}}></div>
                <div className="absolute inset-0 -m-8 rounded-full border border-purple-400 opacity-30" style={{animation:'expand 2s ease-out infinite'}}></div>
                <div className="absolute inset-0 -m-16 rounded-full border border-purple-300 opacity-15" style={{animation:'expand 2s ease-out infinite 0.7s'}}></div>
              </div>
            </div>
            <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 shadow-sm">
              <div className="text-xs font-bold text-purple-700">📍 Vilnius, Lietuva</div>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: 'Bendros pajamos',  val: '$26,300', delta: '+28%', color: 'text-purple-600' },
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={salesData}>
                ...
                <Tooltip/>
                <Bar dataKey="revenue" name="Pajamos" fill="#8b5cf6" radius={[4,4,0,0]}/>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={salesData}>
                ...
                <YAxis tick={{fontSize:11}}/>
                <Tooltip/>
                <Line type="monotone" dataKey="orders" stroke="#8b5cf6" strokeWidth={2} dot={{r:4,fill:'#8b5cf6'}} name="Užsakymai"/>
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
